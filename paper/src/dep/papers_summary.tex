...
\begin{enumerate}
    ...
    \]
    \item Verify that $u_t$ and $p_t$ satisfy the Continiuty Equation:
    \[
    \derivative{t} p_t(x) + \Div(p_t u_t) (x) = 0
    \]
    \begin{proof}
        We have $p_t(x) = p_0(\psi_{t}^{-1}(x))|J(\psi_{t}^{-1}(x))|$ so 

        \[
        \derivative{t} p_t(x) = \derivative{t} p_0(\psi_{t}^{-1}(x))|J(\psi_{t}^{-1}(x))|
        \]
